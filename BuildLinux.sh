...
function check_available_memory_and_disk() {
    ...
    if [ ${FREE_MEM_GB} -le ${MIN_MEM_GB} ]; then
        echo -e "\nERROR: Prusa Slicer Builder requires at least ${MIN_MEM_GB}G of 'available' mem (systen has only ${FREE_MEM_GB}G available)"
        echo && free -h && echo
        exit 2
    fi

    if [[ ${FREE_DISK_KB} -le ${MIN_DISK_KB} ]]; then
        echo -e "\nERROR: Prusa Slicer Builder requires at least $(echo $MIN_DISK_KB |awk '{ printf "%.1fG\n", $1/1024/1024; }') (systen has only $(echo ${FREE_DISK_KB} | awk '{ printf "%.1fG\n", $1/1024/1024; }') disk free)"
        echo && df -h . && echo
        exit 1
    fi
}
...
# Addtional Dev packages for PrusaSlicer
...
then
    ...
    pushd deps/build
        ...
        # rename wxscintilla # TODO: DeftDawg: Does PrusaSlicer need this?
...
then
    ...
    pushd build
        ...
        make -j$NCORES PrusaSlicer # Slic3r
    popd
    ./run_gettext.sh
    echo "done"
fi
